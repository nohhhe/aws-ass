...
**Object Lock** — 한 번 쓰면 수정/삭제를 막는 WORM(Write Once Read Many, 한 번 쓰고 여러 번 읽기) 잠금:
- Versioning 켠 버킷에서만 사용, 기존 버킷에도 나중에 활성화 가능 (한 번 활성화하면 해제 불가)
- **보관 모드 두 가지**:
  - **Governance**: 특권 IAM이 해제 가능
  - **Compliance**: **root 계정조차도 수정/삭제 불가**, 보관 기간 단축도 불가 → 가장 엄격
- **Legal Hold (법적 보류)**: 기간 설정 없이 "이 객체는 홀드" 상태 유지, 수사/소송 대비

> **참고 자료 정정 — Object Lock 활성화 시점**
> 기존 문서에서 "버킷 생성 시 또는 이후에 걸 수 있음"이라고 서술했는데, 이것은 맞음. 과거에는 생성 시에만 가능했으나 AWS가 업데이트하여 **기존 버킷에도 활성화 가능**해짐. 단, 한 번 활성화하면 해제 불가.
> 참고: https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-lock-configure.html

**Glacier Vault Lock** — 아카이브 단위 WORM:
- Vault 정책은 한 번 잠그면 변경 불가 (Compliance와 같은 수준)

**시험 판별**:
- "규정상 누구도 삭제 불가" → **Object Lock Compliance** 또는 **Glacier Vault Lock**
...
- 파트 단위 병렬 업로드 → 속도 향상, 실패한 파트만 재전송, 총 **5TB**까지
...
- 병렬로 여러 바이트 범위를 동시에 가져와 다운로드 가속

### 1.14 S3 VPC 접근 (시험 매우 빈출)

**게이트웨이 엔드포인트 (Gateway Endpoint)**:
- VPC 안의 EC2가 인터넷을 거치지 않고 S3에 직접 접근할 수 있게 해주는 **무료** 통로
- 원래 EC2에서 S3에 접근하려면 인터넷 게이트웨이를 거쳐야 하는데, Gateway Endpoint를 만들면 VPC 내부 네트워크만으로 S3에 도달 가능 → 보안 강화 + 트래픽이 인터넷에 노출되지 않음
...
**시험 판별**:
- VPC 내부 EC2 → S3 프라이빗 접근 → **Gateway Endpoint** (무료, 대부분 정답)
- 온프레미스 → Direct Connect/VPN → S3 → **Interface Endpoint**
- "인터넷을 거치지 않고 S3 접근" → VPC Endpoint
