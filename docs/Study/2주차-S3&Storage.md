...
> 참고 — 버킷 한도 시점 차이
> 2024년 11월 AWS 발표로 현재 General Purpose Bucket 기본 한도는 계정당 10,000개, 상향 요청 시 최대 1,000,000개까지 가능. 다만 SAA-C03 시험은 여전히 "100/1,000"으로 출제되는 경향이라 시험 대비용으로는 기존 수치 유지.
> 참고: https://aws.amazon.com/about-aws/whats-new/2024/11/amazon-s3-up-1-million-buckets-per-aws-account/
...
> 참고 — 객체 최대 크기 시점 차이
> SAA-C03 시험은 "단일 객체 최대 5TB"로 출제. 그러나 AWS 공식 문서 기준 현재 단일 객체 최대 크기는 48.8 TiB (멀티파트 업로드 시). 단일 PUT 한도 5GB는 동일.
> 참고: https://docs.aws.amazon.com/AmazonS3/latest/userguide/qfacts.html
...
```mermaid
graph LR
    subgraph 자주접근["자주 접근 (저장 비쌈, 꺼내기 쌈)"]
        STD["Standard"]
        IT["Intelligent-<br/>Tiering"]
    end
    subgraph 가끔접근["가끔 접근"]
        SIA["Standard-IA"]
        OZIA["One Zone-IA"]
    end
    subgraph 아카이브["아카이브 (저장 쌈, 꺼내기 비쌈/느림)"]
        GI["Glacier<br/>Instant"]
        GF["Glacier<br/>Flexible"]
        GDA["Glacier<br/>Deep Archive"]
    end

    STD --> SIA --> GI --> GF --> GDA

    style STD fill:#4CAF50,color:#fff
    style IT fill:#8BC34A,color:#fff
    style SIA fill:#FFC107,color:#000
    style OZIA fill:#FF9800,color:#fff
    style GI fill:#2196F3,color:#fff
    style GF fill:#1565C0,color:#fff
    style GDA fill:#0D47A1,color:#fff
```
...
#### Glacier Flexible Retrieval 복원 옵션 (시험 빈출)

💡 같은 Glacier 클래스라도 급할수록 비용이 비쌈:
...
```mermaid
flowchart LR
    Upload["객체 업로드<br/>(Standard)"]
    IA["Standard-IA<br/>(30일 후)"]
    Glacier["Glacier Flexible<br/>(90일 후)"]
    Delete["영구 삭제<br/>(365일 후)"]

    Upload -- "30일 경과" --> IA
    IA -- "90일 경과" --> Glacier
    Glacier -- "365일 경과" --> Delete

    style Upload fill:#4CAF50,color:#fff
    style IA fill:#FFC107,color:#000
    style Glacier fill:#2196F3,color:#fff
...
sequenceDiagram
    ...
    participant S3 as S3 버킷 (Versioning ON)

    User->>S3: ① PUT photo.jpg (v1)
    Note over S3: photo.jpg v1 저장

    User->>S3: ② PUT photo.jpg (덮어쓰기)
    Note over S3: photo.jpg v2 저장<br/>v1도 그대로 보관

    User->>S3: ③ DELETE photo.jpg
    Note over S3: Delete Marker 추가<br/>v1, v2 모두 그대로 보관<br/>(겉보기엔 삭제됨)

    User->>S3: ④ Delete Marker 삭제
    Note over S3: v2가 최신 버전으로 복원
```
...
```mermaid
flowchart LR
    subgraph Transit["전송 중 암호화"]
        HTTPS["HTTPS<br/>(TLS)"]
    end
    subgraph AtRest["저장 시 암호화"]
        subgraph SSE["서버 측 암호화 (SSE)"]
            S3Key["SSE-S3<br/>(AWS 관리 키)"]
            KMSKey["SSE-KMS<br/>(고객 KMS 키)"]
            CKey["SSE-C<br/>(고객 제공 키)"]
            DKey["DSSE-KMS<br/>(2중 암호화)"]
        end
        CSEBlock["CSE<br/>(클라이언트 측 암호화)"]
    end

    Client["클라이언트"] -- "암호화된 채널" --> HTTPS
    HTTPS --> S3["S3 저장소"]
    S3 --> SSE
    Client -- "암호화 후 업로드" --> CSEBlock --> S3
```
...
```mermaid
flowchart TB
    Req["S3 요청 들어옴"] --> BPA{"Block Public<br/>Access?"}
    BPA -- "퍼블릭 요청 + ON" --> Deny1["❌ 거부"]
    BPA -- "통과" --> ExDeny{"명시 Deny<br/>(IAM/Bucket Policy)?"}
    ExDeny -- "Deny 있음" --> Deny2["❌ 거부"]
    ExDeny -- "없음" --> SCP{"조직 SCP<br/>제한?"}
    SCP -- "제한됨" --> Deny3["❌ 거부"]
    SCP -- "통과" --> Allow{"IAM 또는<br/>Bucket Policy에<br/>Allow?"}
    Allow -- "Allow 있음" --> OK["✅ 허용"]
    Allow -- "없음" --> Deny4["❌ 거부<br/>(기본 거부)"]
```
...
flowchart LR
    ...
    ACM["ACM 인증서<br/>(us-east-1)"]
    OAC["OAC<br/>(접근 제어)"]
    S3["S3 버킷<br/>(정적 파일)"]

    User -- "example.com 요청" --> R53
    R53 -- "A Alias → CF" --> CF
    ACM -. "HTTPS 인증서" .-> CF
    CF -- "OAC로 인증" --> OAC --> S3
    ...
    style R53 fill:#4CAF50,color:#fff
```
...
flowchart TB
    subgraph Seoul["서울 리전"]
        SrcBucket["원본 버킷<br/>(Versioning ON)"]
    end
    subgraph Tokyo["도쿄 리전"]
        DstCRR["대상 버킷 (CRR)<br/>재해 복구용"]
    end
    subgraph SeoulSame["서울 리전 (다른 버킷)"]
        DstSRR["대상 버킷 (SRR)<br/>감사/분석용"]
    end

    SrcBucket -- "CRR (교차 리전 복제)" --> DstCRR
    SrcBucket -- "SRR (동일 리전 복제)" --> DstSRR
```

- **CRR (Cross-Region Replication, 교차 리전 복제)**: 다른 리전의 버킷으로 복제
- **SRR (Same-Region Replication, 동일 리전 복제)**: 같은 리전의 다른 버킷으로 복제. 💡 같은 데이터를 용도별로 분리 관리할 때 사용. 예: 여러 팀의 로그를 분석용 버킷 한 곳에 모으기, 운영팀 버킷과 감사팀 버킷을 분리해서 권한 관리, 프로덕션 데이터를 테스트 환경 버킷에 복제
...
- AWS 계정이 없는 외부 사용자에게 특정 객체 1개에 대해 시간 제한이 있는 URL을 발급하여 임시 접근을 허용하는 기능. 예를 들어 유료 콘텐츠 다운로드 링크, 고객이 파일을 업로드할 수 있는 임시 URL 등에 활용
...
> 기존 문서에서 "최대 7일(IAM User) / 36시간(IAM Role)"이라고 서술했으나, IAM Role의 경우 고정 36시간이 아니라 임시 자격 증명의 유효 기간까지가 정확함. URL 만료 = min(설정한 만료 시간, 자격 증명 만료 시간).
...
```mermaid
flowchart LR
    subgraph Without["Gateway Endpoint 없이"]
        EC2a["EC2<br/>(프라이빗)"] --> NATa["NAT Gateway"] --> IGWa["IGW"] --> Inet["공인 인터넷"] --> S3a["S3"]
    end

    subgraph With["Gateway Endpoint 사용 시"]
        EC2b["EC2<br/>(프라이빗)"] --> GWE["Gateway<br/>Endpoint"] --> S3b["S3"]
    end

    style Inet fill:#f44336,color:#fff
    style GWE fill:#4CAF50,color:#fff
```
...
💡 Gateway Endpoint와 Interface Endpoint는 둘 다 "VPC Endpoint"의 하위 유형이다. 둘 다 공인 인터넷을 거치지 않는 건 동일하고, 용도가 다를 뿐.
...
**S3 Inventory**: 버킷에 있는 객체들의 목록과 메타데이터(크기, 스토리지 클래스, 암호화 상태 등)를 매일/매주 CSV 또는 Parquet 파일로 자동 생성. "우리 버킷에 뭐가 얼마나 있나" 파악하거나, Batch Operations의 입력으로 활용
...
flowchart TB
    ...
    style EFS fill:#FF9800,color:#fff
```
...
- Mount Target: EFS에 접근하기 위한 연결 지점. 각 AZ에 하나씩 만들어야 하며, EC2는 자기가 속한 AZ의 Mount Target을 통해 EFS에 연결. Mount Target이 없는 AZ의 EC2는 EFS에 접근할 수 없음
- 여러 AZ에 중복 저장 → 하나의 AZ가 파괴되더라도 다른 AZ에서 서비스 가능
- VPN 또는 Direct Connect를 통해 온프레미스에서 접속 가능
...
- 💡 저장 시 암호화: KMS 키를 사용. 파일 시스템(EFS 전체) 단위로 설정하며, 최초 생성 시에만 켤 수 있음. 켜면 그 EFS에 저장되는 모든 파일이 자동 암호화됨
- 💡 이미 만들어진 미암호화 EFS를 나중에 암호화로 변경할 수 없음 → 새 EFS를 암호화 ON으로 생성하고, DataSync로 데이터를 옮겨야 함

### 2.6 EFS Access Points

- 💡 Access Point를 만들 때 "이 Access Point로 접근하면 루트 디렉토리는 /app-a, 사용자는 uid=1000"처럼 미리 설정해둠. 앱 A에게는 Access Point A의 주소를, 앱 B에게는 Access Point B의 주소를 알려주면, 각 앱이 서로 다른 디렉토리와 권한으로 접근하게 됨
- IAM 정책으로 Access Point별 접근 제어 가능
- 사용 예: 하나의 EFS를 여러 앱이 공유하되, 각 앱이 자기 디렉토리만 보게 격리
...
## 3. Amazon FSx (File System x)

### 3.1 FSx란?

💡 EFS는 Linux의 NFS만 지원한다. 하지만 현실에서는 Windows SMB 파일 공유가 필요하거나, 초고성능 HPC용 파일 시스템(Lustre)이 필요하거나, 온프레미스에서 쓰던 NetApp/ZFS를 그대로 AWS로 옮기고 싶은 경우가 있다. 이런 다양한 파일 시스템 수요를 위해 AWS가 제공하는 것이 FSx.
...
- Linux에서 실행되는 고성능 컴퓨팅(HPC, High Performance Computing) 워크로드에 적합 — 💡 S3는 HTTP API로 객체를 하나씩 가져오는 방식이라 대량의 파일을 빠르게 읽고 쓰는 데 느림. Lustre는 POSIX 파일 시스템이라 로컬 디스크처럼 직접 읽기/쓰기가 가능하고, 수백 GB/s 처리량을 낼 수 있음. 과학 시뮬레이션, 금융 모델링, 머신러닝 학습 등에 사용
- POSIX 호환, Linux 인스턴스 및 컨테이너에서 접근
- S3와 연동: 💡 S3에 원본 데이터를 두고, Lustre가 그걸 고속 파일 시스템으로 캐시해서 처리하는 구조. 데이터 자체는 S3에 있고, Lustre는 "S3 데이터를 빠르게 읽고 쓸 수 있는 고속 캐시 레이어" 역할. 처리가 끝나면 결과를 S3에 다시 쓰고 Lustre는 삭제할 수 있음

```mermaid
flowchart LR
    S3["S3 버킷<br/>(원본 데이터)"]
    Lustre["FSx for Lustre<br/>(고속 캐시 레이어)"]
    EC2["EC2 인스턴스<br/>(HPC/ML 처리)"]

    S3 -- "Lazy Loading<br/>(접근 시 가져옴)" --> Lustre
    Lustre -- "Write-back<br/>(결과 저장)" --> S3
    EC2 <-- "POSIX 마운트<br/>(로컬 디스크처럼)" --> Lustre
```

  - Lazy Loading (지연 로딩): 처음 접근할 때만 S3에서 가져옴 (전체 복사 X)
  - Write-back (쓰기 반영): 처리 결과를 S3에 다시 저장
- 💡 배포 유형:
...
- 💡 OpenZFS: NFS 프로토콜, Linux 기반, 스냅샷/클론 지원, ZFS 마이그레이션에 적합. 💡 Lustre와 비교하면 Lustre는 HPC용 초고속 병렬 처리(수백 GB/s)이고, OpenZFS는 일반 서버 워크로드에서의 빠른 NFS(수 GB/s) + 스냅샷/클론이 강점. 용도가 다름
...
```mermaid
flowchart LR
    Order["① AWS 콘솔에서<br/>장비 주문"]
    Ship["② AWS가<br/>장비 배송"]
    Load["③ 현장에서<br/>데이터 로드"]
    Return["④ AWS로<br/>장비 반송"]
    Upload["⑤ S3에<br/>데이터 업로드"]

    Order --> Ship --> Load --> Return --> Upload
```

### 4.2 Snow Family가 필요한 이유

- 네트워크로 전송하기엔 데이터 양이 너무 많을 때. 💡 예: 100TB를 1Gbps 인터넷으로 전송하면 약 12일이 걸리지만, Snowball Edge로 보내면 장비 배송 포함 약 1주일이면 완료
- 네트워크 연결이 불가능하거나 대역폭이 부족한 환경
- 엣지 컴퓨팅: 데이터가 발생하는 현장(공장, 광산, 선박 등)에서 AWS 클라우드에 보내지 않고 장비 내에서 직접 데이터를 처리하는 것. 인터넷이 느리거나 없는 원격지에서 실시간 처리가 필요할 때 사용

### 4.3 Snowcone

- Snow Family 중 가장 작은 장비
- 💡 HDD: 8TB / SSD: 14TB
- DataSync 에이전트 사전 설치: DataSync는 AWS에서 제공하는 데이터 이동 서비스로, 네트워크를 통해 온프레미스 ↔ AWS 간 데이터를 전송하는 도구. Snowcone에는 이 에이전트가 미리 설치되어 있어서, 네트워크 연결이 가능하면 장비를 반송하지 않고 **온라인으로 전송 가능** (Snow Family 중 유일 — 시험 단서)
...
> 참고 — Snowball Edge 사양 시점 차이
> SAA-C03 시험은 위 수치(80TB / 42TB)로 출제. 현재 AWS 공식 문서의 차세대 기기 사양은 Storage Optimized 210TB NVMe, Compute Optimized 28TB NVMe (GPU 옵션은 현 사양에 명시 없음)로 변경됨. 또한 AWS Snowball은 신규 고객 대상 신청이 제한되는 추세.
> 참고: https://docs.aws.amazon.com/snowball/latest/developer-guide/device-differences.html
...
> 참고 — Snowmobile 서비스 종료
> AWS Snowmobile은 2024년 4월 서비스 종료(discontinued). SAA-C03 시험에는 아직 출제되므로 개념·용량(100PB)은 학습 유지하되, 실무 권장 옵션은 아님. 현재는 Snowball Edge 다수 또는 Direct Connect/네트워크 전송으로 대체.
> 참고: https://aws.amazon.com/blogs/aws/
...
- 클래스 7종: Standard(매일 접근) / Intelligent-Tiering(패턴 예측 불가, 자동 최적화) / Standard-IA(월 1~2회) / One Zone-IA(IA인데 손실 OK) / Glacier Instant(분기별, 즉시 복원) / Glacier Flexible(연 1~2회, 분~시간 복원) / Glacier Deep Archive(거의 안 봄, 12~48시간 복원)
- Glacier Flexible 복원: Expedited(1-5분, 비쌈) / Standard(3-5시간) / Bulk(5-12시간, 저렴)
- Lifecycle: 시간 경과에 따라 자동으로 클래스 전환/삭제. 시험 단골: "30일 → IA, 90일 → Glacier, 365일 → Deep Archive"
- Versioning: 덮어쓰기/삭제해도 이전 버전 보관. MFA Delete로 영구 삭제 방지. 기능 자체는 무료지만 버전마다 저장 비용 발생
- 암호화: SSE-S3(기본, 무료) / SSE-KMS(감사 기록 필요 시) / SSE-C(키를 AWS에 안 맡김) / DSSE-KMS(2중 암호화) / CSE(AWS에 평문 안 보냄)
...
- 복제: CRR(교차 리전, DR/지연 최소화) / SRR(동일 리전, 용도별 분리). Versioning 필수
- Object Lock: Compliance(보관 기간 동안 root도 삭제 불가) / Governance(특권 IAM 해제 가능) / Legal Hold(기간 없이 보류)
- Pre-signed URL: AWS 계정 없는 외부 사용자에게 시간 제한 임시 접근 허용
- Transfer Acceleration: 원거리 대용량 업로드 가속 (엣지 경유, 추가 비용)
- VPC → S3 프라이빗 접근: Gateway Endpoint(무료, VPC 내부) / Interface Endpoint(유료, 온프레미스)
- S3 Event → Lambda/SQS/SNS/EventBridge

### EFS 핵심

- NFS v4 기반, Linux 전용, 다수 EC2 동시 접근, 자동 확장, 다중 AZ
- 각 AZ에 Mount Target 필요 (없는 AZ의 EC2는 접근 불가)
- 성능 모드: General Purpose(기본, 저지연) / Max I/O(고처리량, 고지연)
- 처리량 모드: Bursting(기본, 용량 비례) / Elastic(권장, 자동 조절) / Provisioned(미리 지정)
- 암호화: 파일 시스템 단위, 최초 생성 시에만 설정 가능
- Windows 파일 공유 → FSx for Windows, Linux 공유 폴더 → EFS

### FSx 핵심

- Windows + SMB + AD → FSx for Windows File Server
- Linux + HPC + S3 연동 → FSx for Lustre (S3의 고속 캐시 레이어, 수백 GB/s)
- 멀티프로토콜 (NFS+SMB+iSCSI) → FSx for NetApp ONTAP (온프레미스 NetApp 마이그레이션)
- Linux + 스냅샷/클론 → FSx for OpenZFS (일반 서버 워크로드, 수 GB/s)

### Snow Family 핵심

- 대용량 오프라인 마이그레이션: Snowcone(8-14TB) < Snowball Edge(42-80TB) < Snowmobile(100PB)
- Snowball Edge: Storage Optimized(80TB, 데이터 전송) / Compute Optimized(42TB + GPU, 엣지 컴퓨팅)
- Snowcone: DataSync 온라인 전송 가능 (Snow Family 중 유일)
- Snowmobile: 10PB 이상 시 효율적
- 엣지 컴퓨팅: EC2/Lambda를 Snow 장비에서 로컬 실행 가능

### 💡 S3 vs EBS vs EFS vs FSx — 언제 뭘 쓰나?

S3만 쓰면 안 되는 이유: S3는 HTTP API로 객체를 주고받는 저장소라서, EC2에서 로컬 디스크처럼 마운트(`/data/file.txt`로 접근)할 수 없음. DB나 앱이 파일 경로로 데이터에 접근해야 하면 EBS나 EFS가 필요.

```mermaid
flowchart TB
    Q1{"파일 경로 접근<br/>필요?"}
    Q1 -- "아니오<br/>(HTTP API OK)" --> S3["S3<br/>백업/아카이브/데이터레이크"]
    Q1 -- "예" --> Q2{"여러 EC2에서<br/>공유 필요?"}
    Q2 -- "아니오<br/>(단일 EC2)" --> EBS["EBS<br/>DB/부트볼륨"]
    Q2 -- "예" --> Q3{"OS?"}
    Q3 -- "Linux" --> Q4{"초고속 HPC?"}
    Q4 -- "아니오" --> EFS["EFS<br/>공유 파일"]
    Q4 -- "예" --> Lustre["FSx for Lustre<br/>HPC/ML"]
    Q3 -- "Windows" --> FSxW["FSx for Windows<br/>SMB + AD"]
```

| 스토리지 | 접근 방식 | 공유 | 언제 쓰나 |
|--------|--------|-----|---------|
| S3 | HTTP API (업로드/다운로드) | 무제한 클라이언트 | 백업, 정적 파일, 아카이브, 데이터 레이크. 파일 경로 접근이 필요 없는 경우 |
| EBS | EC2에 마운트하는 블록 디스크 | 단일 EC2 (io2 Multi-Attach 제외) | DB, OS 부트 볼륨, 한 EC2 전용 스토리지 |
| EFS | 여러 EC2가 마운트하는 공유 폴더 (NFS) | 다수 EC2 동시 접근 | 여러 EC2가 같은 파일을 공유해야 할 때. Linux 전용 |
| FSx for Windows | Windows 공유 폴더 (SMB) | 다수 Windows 서버 | Windows 환경에서 파일 공유, Active Directory 연동 |
| FSx for Lustre | 초고속 병렬 파일 시스템 | 다수 Linux 서버 | HPC, ML 학습 등 S3 데이터를 초고속으로 처리해야 할 때 |
